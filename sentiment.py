...
def final_engine_score(structure, mag7, news, social, risk) -> dict:
    ...
    if bullish >= 75:
        mode = "Strong Bullish Bias — Longs preferred, avoid chasing."
    elif bullish >= 60:
        mode = "Bullish Bias — Longs preferred only at quality pullbacks."
    elif bullish >= 45:
        mode = "Neutral / Chop — No trade or reduced size."
    elif bullish >= 25:
        mode = "Bearish Bias — Shorts preferred only at quality rallies."
    else:
        mode = "Strong Bearish Bias — Shorts preferred, avoid chasing."

    trade_permission = "TRADE ALLOWED WITH CONFIRMATION"
    if 45 <= bullish <= 55:
        trade_permission = "NO TRADE — mixed signal zone"
    if risk["points"] <= 3:
        trade_permission = "NO TRADE — risk conditions poor"
